...
class modelo_controller extends Controller
{
    ...
    {   
        $variables = DB::select('SELECT * FROM variables');
        //dd($variables[1]->nombre);
        $arreglo = array();
        for ($i=0; $i < count($variables); $i++) { 
            $arreglo[$variables[$i]->variable] = $variables[$i]->nombre;
        }

        return view('pagina.modelo')->with('arreglo', $arreglo);
    }
    ...
    {  
        $cont = DB::Select("SELECT AUTO_INCREMENT FROM information_schema.tables WHERE TABLE_SCHEMA = 'porcinos' AND TABLE_NAME = 'modelo'");
        $cont = $cont[0]->AUTO_INCREMENT;

        $array = array();

        //d($request->all(), $request->txtnovariables, $request->txtnorestricciones);
        $txtx1 = "1";
        $txtx2 = "2";
        $txtr1x1 = "3";
        $txtr1x2 = "4";
        ...
        DB::insert('INSERT INTO modelo(id_modelo, nombre, descripcion, nvariables, nrestricciones, accion) VALUES (?, ?, ?, ?, ?, ?)', [$cont, $request->modelnombre, $request->modelodescripcion, $request->txtnovariables, $request->txtnorestricciones, $request->objetivo]);

        DB::insert('INSERT INTO modelo_resultado(id_modelo, variable, valor) VALUES (?, ?, ?)', [$cont, "Z", $array['txtz']]);

        for($i = 1; $i <= $request->txtnovariables; $i++){
          DB::insert('INSERT INTO modelo_variables(id_modelo, variable, valor) VALUES (?, ?, ?)', [$cont, "x".$i, $array['txtx'.$i]]);
      }

      for($i = 1; $i <= $array['contx']; $i++){
          DB::insert('INSERT INTO modelo_resultado(id_modelo, variable, valor) VALUES (?, ?, ?)', [$cont, "x".$i, $array['txtsx'.$i]]);
      }

      for($i = 1; $i <= $array['conts']; $i++){
          DB::insert('INSERT INTO modelo_resultado(id_modelo, variable, valor) VALUES (?, ?, ?)', [$cont, "s".$i, $array['txtss'.$i]]);
      }

      for($i = 1; $i <= $array['conta']; $i++){
          DB::insert('INSERT INTO modelo_resultado(id_modelo, variable, valor) VALUES (?, ?, ?)', [$cont, "a".$i, $array['txtsa'.$i]]);
      }
...
Flash::success("Se ha Almacenado el modelo " . $request->modelnombre);
